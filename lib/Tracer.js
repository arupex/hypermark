...
class Tracer {
    ...
    startTrace (context) {
        return new Trace(this.__traces.reduce((acc, trace) => {
            ...
            acc.push({
                value : value,
                request : Array.isArray(cTrace.request)?cTrace.request:typeof cTrace.request === 'string'?[cTrace.request]:[],
                response : Array.isArray(cTrace.response)?cTrace.response:typeof cTrace.response === 'string'?[cTrace.response]:[],
                log : Array.isArray(cTrace.log)?cTrace.log:typeof cTrace.log === 'string'?[cTrace.log]:[]
            });
            return acc;
        }, []));
    }
